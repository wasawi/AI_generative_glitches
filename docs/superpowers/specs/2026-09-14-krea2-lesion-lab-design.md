...
| `strength` | FLOAT | `0.15` | 0–10, step 0.01 | Dose; `0` is always a no-op |
...
|---|---|---|
| `dropout` | `a × (1 − s)` | 0–1 (1 = silenced) |
| `amplify` | `a × (1 + s)` | 0–10 |
| `sign_flip` | `a × (1 − 2s)` | 0–1 (0.5 = zeroed, 1 = negated) |
| `noise` | `a + s × rms × n` | 0–10 |
...
- recipe: defaults, every validation message, per-mode strength limits, no-op reasons, `describe()` format.
